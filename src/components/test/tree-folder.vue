...
<script>
  ...
  // 我们知道制造矛盾的是 tree-folder-contents 子组件，
  // 所以我们在 tree-folder 组件的生命周期钩子函数 beforeCreate 中去
  // 注册 tree-folder-contents 组件」
  export default {
    props: ['folder'],
    data: function () {
      return {}
    ...
    beforeCreate: function () {
      ...
      // 此句指在调用之前便声明了tree-folder-contents子组件（如果由runder后调用则会报错，因为没有定义）
